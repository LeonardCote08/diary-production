const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/viewerEventHandlers-CxCdoM9a.js","assets/main-nUf9gQDL.js","assets/main-iSp7nxPb.css"])))=>i.map(i=>d[i]);
var __defProp = Object.defineProperty;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
import { O as OpenSeadragon, i as isMobile, g as getBrowserOptimalDrawer, a as applyTileCascadeFix, b as getTuningState, c as OverlayManagerFactory, d as applyTuningToViewer, _ as __vitePreload, r as removeTileCascadeFix } from "./main-nUf9gQDL.js";
...
function createIOSHTMLConfig(baseConfig) {
  ...
  return {
    ...
    maxImageCacheCount: isIPhone() ? 50 : isIPad() ? 400 : 75,
    // CRITICAL: Increased cache to prevent re-fetching (was 100)
    imageLoaderLimit: isIPhone() ? 2 : 2,
    // CRITICAL: Reduced for HTML drawer (was 4)
    maxTilesPerFrame: isIPhone() ? 3 : 1,
    // CRITICAL: HTML drawer performs better with sequential loading (was 5)
    ...
    animationTime: 0.4,
    // CRITICAL: Match tile loading time (was 0.15)
    springStiffness: 10,
    // Tighter control
    blendTime: 0,
    // CRITICAL: Eliminate fade showing black background (was 0.2)
    // Tile settings
    tileSize: 512,
    tileOverlap: 1,
    ...
    preload: false,
    // CRITICAL: Disable during navigation (was true)
    // Debug info
    debugMode: false,
    showNavigator: false
  };
...
function applyIOSHTMLStyles() {
  ...
  style.textContent = `
        ...
        .openseadragon-tile {
            ...
            /* CRITICAL: will-change removed - causes memory pressure with 24+ tiles */
        }
        ...
        .openseadragon-tile img {
            ...
            decoding: async !important;   /* CRITICAL: Async decoding prevents main thread blocking (was sync) */
        }

        /* Optimize tile container for HTML rendering */
        .openseadragon-tile {
            contain: layout style paint;  /* CSS containment for performance */
            /* will-change removed - let browser optimize naturally */
        }

        /* Conditional will-change only during active animation */
        .openseadragon-container.animating .openseadragon-tile {
            will-change: transform;  /* Apply only during animation */
        }

        /* Force eager loading on all images */
        img[src] {
            loading: eager !important;
            decoding: async !important;  /* Changed from sync to async */
        }
    `;
...
const buildViewerConfig = (config, dziUrl, drawerType, isMobileDevice, tileSourceConfig = null) => {
  ...
  const finalConfig = {
    ...
    gestureSettingsTouch: {
      ...
      dblClickToZoom: false,
      flickEnabled: true,
      // MOBILE UX FIX: Re-enabled on iOS for native feel
      flickMinSpeed: config.flickMinSpeed,
      flickMomentum: isIOS2 ? 0.3 : 0.8,
      // MOBILE UX FIX: Conservative momentum on iOS (0.3 to minimize tile corruption), standard on Android
      pinchToZoom: true,
      dragToPan: true,
      pinchRotate: false
      // Research: Disable for performance
...
class SimplifiedPerformanceMode {
  ...
  enable(viewer) {
    ...
    if (viewer) {
      ...
      if (isMobile2) {
        console.log(
          "[SimplifiedMode] Mobile cache PRESERVED (black tiles fix) - current:",
          viewer.maxImageCacheCount
        );
      }
...
async function initializeViewer(viewerRef, props, state, handleHotspotClick) {
  ...
  const eventHandlers = await __vitePreload(() => import("./viewerEventHandlers-CxCdoM9a.js"), true ? __vite__mapDeps([0,1,2]) : void 0);
  ...
  if (isIOS2) {
    console.log("[Phase 2] Installing dynamic iPad tile loading optimization");
    let navTimeout;
    const reduceLoadingDuringNav = () => {
      if (viewer.imageLoader) {
    ...
    viewer.addHandler("canvas-drag", reduceLoadingDuringNav);
    viewer.addHandler("zoom", reduceLoadingDuringNav);
    ...
    console.log("[Phase 2] Dynamic optimization installed");
  }
