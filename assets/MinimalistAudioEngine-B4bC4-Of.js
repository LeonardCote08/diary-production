import { g as getHapticManager } from "./HapticManager-CwcpB4W7.js";
import "./main-D6EELDcY.js";
class DopamineAudioEngine {
  constructor() {
    ...
    this.variations = {
      ...
      activation: [
        ...
        // Absorbed Impact (son par défaut mobile)
      ]
    };
  ...
  playReveal(x = 0, y = 0) {
    ...
    if (this.useAbsorbedImpactForBoth) {
      ...
      console.log("[DopamineAudioEngine] Playing Absorbed Impact ★ for REVEAL");
  ...
  // Future method for switching between different tap sounds
  // Will be populated with sounds from research (Soft Wood, Ink Drop, etc.)
  setTapSound(soundName) {
    if (soundName === "absorbed") {
      this.currentVariation.activation = 3;
      this.useAbsorbedImpactForBoth = true;
      console.log("[DopamineAudioEngine] Using Absorbed Impact");
    }
  }
  // Get current tap sound name
  getCurrentTapSound() {
    return "absorbed";
  }
  // A/B Testing System
...
class MinimalistAudioEngine extends DopamineAudioEngine {
  ...
  playReveal(x, y) {
    ...
    if (this.hapticsEnabled) {
      this.haptics.reveal();
      console.log("[MinimalistAudio] Triggered reveal haptic (50ms)");
    }
  }
  ...
  playActivate() {
    ...
    if (this.hapticsEnabled) {
      ...
      console.log("[MinimalistAudio] Triggered activate haptic (70ms)");
    }
  }
  ...
  // Method to switch between different tap sounds (future sounds)
  setTapSound(soundName) {
    super.setTapSound(soundName);
  }
  // Get current tap sound name
  getCurrentTapSound() {
    return super.getCurrentTapSound();
  }
